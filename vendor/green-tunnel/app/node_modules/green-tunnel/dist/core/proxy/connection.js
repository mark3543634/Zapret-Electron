 ...
 */
export async function handleConnection(client, ctx) {
...
async function readRequestHead(socket, timeoutMs) {
...
export async function readChunk(socket, timeoutMs) {
    socket.resume();
    try {
        for await (const chunk of readChunks(socket, timeoutMs)) {
            return chunk;
 ...
 */
function readChunks(socket, timeoutMs) {
...
function respond(socket, status, reason, body) {

 ...
 */
export async function tunnel(client, server, idleTimeoutMs, transform) {
    ...
    try {
        ...
        const failure = results.find((result) => result.status === 'rejected' && !isBenign(result.reason));
        if (failure?.status === 'rejected') {
            throw toError(failure.reason);
        }
    }
    finally {
        for (const off of disarm)
            off();
